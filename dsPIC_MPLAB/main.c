...
void TaskPWM(void *params) {
    ...
    for (;;) {
        ...
        // Actualizare text doar dacã s-a schimbat
...
unsigned int read_adc(void)
{
    AD1CHS0bits.CH0SA = 3; // Selectãm canalul AN3 (RB3)
    AD1CON1bits.SAMP = 1;  // Pornim e?antionarea
    while (!AD1CON1bits.DONE); // A?teptãm conversia
    return ADC1BUF0;      // Returnãm valoarea ADC cititã
}
